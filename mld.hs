...
mld :: RMCode -> V8 Z2 -> V8 Z2
mld rm z = stage1 rm z

stage1 :: (Eq [Int]) => RMCode -> V8 Z2 -> V8 Z2
stage1 rm@(RM r m) z = let 
  majs = map stage1_1 (vectorSpaceBases r :: [[V3 Z2]]) 
  res = stage1_3 $ map stage1_1 (vectorSpaceBases r :: [[V3 Z2]]) in res where
  stage1_1 m_vs = (,) (tr (chi m_vs)) $ moreOne $ map stage1_2 $ take (2 ^ (m - r) - 2) $ zip [1..] $ filter (containsAll m_vs) $ vectorSpaceBases (r+1)
  stage1_2 (i, mi) = let
    ki = (chi mi) ⋅ z in
    ki
  stage1_3 majs = stage2 1 majs rm z

stage2 :: (Eq [Int]) => Int -> [([Int], Bool)] -> RMCode -> V8 Z2 -> V8 Z2
...
stage3 :: (Eq [Int]) => Int -> [([Int], Bool)] -> RMCode -> V8 Z2 -> V8 Z2
